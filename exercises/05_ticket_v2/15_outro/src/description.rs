...
impl TryFrom<String> for TicketDescription {
    ...
    fn try_from(value: String) -> Result<Self, Self::Error> {
        if value.is_empty(){ return Err(ParseDescriptionError::DescriptionCannotBeEmpty) }
        else if value.len() > 500 { return Err(ParseDescriptionError::DescriptionTooLong)}
        else { return Ok(Self(value)) }
    }
}
...
impl TryFrom<&str> for TicketDescription {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        ...
        else { return Ok(Self(value.into())) }
    }
}
